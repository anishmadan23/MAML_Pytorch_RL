...
def parse_args():
    ...
    parser.add_argument('--fast-lr', type=float, default=0.1,
                        help='learning rate for the 1-step gradient update of MAML/CAVIA')              # changes from 1
    

    ## For PPO

    parser.add_argument('--meta-lr',type=float,default=0.1, help='To be used only for PPO')
    parser.add_argument('--eps_clip',type=float,default=0.1, help='Clipping for the PPO objective')
    parser.add_argument('--critic_weight',type=float,default=0.5, help='Weight for the critic loss term in the multi-loss function')
    parser.add_argument('--entropy_wt',type=float,default=0.01, help='Weight for entropy term in loss function')
