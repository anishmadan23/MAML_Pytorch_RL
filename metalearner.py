...
class MetaLearner(object):
    ...
    def test_sample_mj(self, tasks, num_steps, batch_size, halve_lr):
        """Sample trajectories (before and after the update of the parameters)
        for all the tasks `tasks`.batchsize
        """
        num_steps=3
        ...
        for task in tasks:
            ...
            # get new rollouts
            frames = self.sampler.test_sample(self.policy, gamma=self.gamma, params=params, batch_size=batch_size)

            # test_episodes = self.sampler.sample_mj(self.policy, gamma=self.gamma, params=params, batch_size=batch_size)
            # curr_episodes.append(test_episodes)

            # episodes_per_task.append(curr_episodes)

        self.policy.reset_context()
        return episodes_per_task,frames
